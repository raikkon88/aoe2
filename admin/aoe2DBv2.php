...
class aoe2DBv2 extends SQLite3 {
    ...
    function updateContent($id,$content,$title){
        $update="UPDATE CONTENT set CONTENT='".$content."', title='".$title."' where ID=".$id;
        $this->exec($update);
    }
    ...
    function getContentUpdate($id){

        $select="SELECT CONTENT , TITLE from CONTENT where ID=".$id;
        $query=$this->query($select);
        $result = $this->resultSetToArray($query);
